...
- **Accuracy:** strict `chart-table-v2` semantic cell F1 on the reused synthetic Common300 set is **28.90% for the fair table LoRA**, versus **6.59% for the pinned base** and **26.67% for numeric loss**. The fair LoRA gets only **13/280 exact-numeric charts completely correct**. These are regression results, not representative customer accuracy. A separate three-chart production pilot recovered 15/15 values on crisp synthetic inputs; human correction time remains unmeasured. [Evaluation contract](docs/EVALUATION_CONTRACT.md) · [Strict rescore](release/evaluation-v2/RESULTS.md) · [Readiness plan](docs/REVIEW_READINESS_PLAN.md) · [Historical research results](RESULTS.md) · [Pilot evidence](docs/LAUNCH_EVALUATION.md).
