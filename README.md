...
- **Accuracy:** historical scorer-v1 `common300` results report 38.9% `cell@5_exact` for the table LoRA versus 13.5% for its pinned base. A separate three-chart production pilot recovered 15/15 values on crisp synthetic inputs. Neither is a representative customer benchmark. The old scorer has known correctness gaps; these are not current strict-table scores. See the [versioned evaluation contract](docs/EVALUATION_CONTRACT.md) [initial strict-contract rescore](release/evaluation-v2/RESULTS.md), and [readiness plan](docs/REVIEW_READINESS_PLAN.md). Human correction time remains unmeasured. [Research results](RESULTS.md) · [Claim boundaries](RESULT_TO_CLAIM.md) · [Pilot evidence](docs/LAUNCH_EVALUATION.md).
