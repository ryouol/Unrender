...
- per-tenant stored-byte, outstanding-upload, and upload-bandwidth quotas, with a credit precheck before customer persistence;
- parameterized SQL, foreign keys, explicit transactions, and credit-ledger idempotency;
- atomic public API upload/job/credit reservation and tenant-scoped request-hash idempotency;
...
- production startup fails closed when public registration, demo seeding/replay, HTTP origins, a disabled worker, an unpinned model, or an unapproved provider release is configured.
...
- tenant quotas and authenticated rate counters are local to one SQLite deployment, with no distributed WAF/rate limiter;
