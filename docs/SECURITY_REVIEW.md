...
No open Critical or High repository finding remains. Seven findings were fixed in this branch. Three Medium/Low deployment risks remain explicit controlled-beta gates because they require the chosen edge, storage platform, or an external test environment.
...
- Severity: Medium
- Location: `unrender/product/web.py:127-166`, `create_app`; `unrender/product/config.py:67-88`, `Settings.validate`
...
- Rule ID: JS-URL-001
- Severity: Medium
- Location: `unrender/product/web.py:109-124` and `:464-497`; `unrender/product/static/app.js:760-771`, `buyCredits`
...
- Severity: Medium
- Location: `unrender/product/service.py:818-858`; `unrender/product/web.py:452-462`; `unrender/product/static/app.js:688-758`
...
- Severity: High
- Location: `unrender/product/service.py:725-792`, `ProductService.export`, `_safe_csv`, and `_xlsx`
...
- Severity: Low
- Location: `unrender/product/service.py:209-219`, `ProductService.authenticate`
...
### 8. Body limits and hostile document scanning depend partly on the edge
...
### 9. Application rate limiting is single-node and proxy-sensitive

- Rule ID: FASTAPI-ABUSE-001
- Severity: Low
- Location: `unrender/product/web.py:189-224`; `unrender/product/service.py:1072-1094`, `rate_limit`
...
### 10. Sessions lack user-facing inventory and global revocation

- Rule ID: FASTAPI-SESSION-001
- Severity: Low
- Location: `unrender/product/service.py:234-283`, session methods; browser account UI
