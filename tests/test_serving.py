...
def test_tuning_cannot_touch_common300():
    ...
    with pytest.raises(ValueError, match="not an eligible final study"):
...
def test_failure_inclusive_accuracy_and_unknown_token_throughput():
    ...
    assert summary["quality_failures_as_misses"]["cell_recall"] == 0.5
...
def test_model_invalid_is_not_salvaged_in_failure_inclusive_score():
    ...
    assert result["quality_failures_as_misses"]["cell_recall"] == 0
...
@pytest.mark.parametrize(
    ...
    [
        ...
        '{"chart_type":"bar","series":[{"points":[{"x":"A","y":1},{"x":"A","y":2}]}]}',
        '{"chart_type":"bar","chart_type":"line","series":[]}',
        "```json\n" + json.dumps(CHART) + "\n```",
    ],
)
def test_invalid_output_never_counts_as_valid_or_correct_throughput(raw):
    from unrender.serving.report import summarize
    from unrender.serving.validity import assess

    # Even a stale/incorrectly stored success flag cannot grant quality credit.
    row = {
        "status": "ok",
        "strict_valid": True,
        "raw": raw,
    ...
    result = summarize([row], 1.0)
    assert result["valid"] == result["correct_tables"] == 0
    assert result["valid_requests_per_s"] == result["correct_tables_per_s"] == 0
    assert result["quality_failures_as_misses"]["cell_recall"] == 0


@pytest.mark.parametrize("finish,done", [("length", True), (None, False), ("stop", False)])
def test_complete_json_without_completed_generation_is_not_accepted(finish, done):
    from unrender.serving.validity import assess

    validity, chart = assess(json.dumps(CHART), finish, done)
    assert validity["strict_schema"] and validity["semantic"]
    assert not validity["accepted"] and chart is None
