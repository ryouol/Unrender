"""Rank saved predictions by the primary metric and display raw-output failures."""
...
def dump(predictions: str, n: int, tol: float, out: str = "") -> Path:
    ...
    scored = []
    for r in validate_rows(load_predictions(pred_path)):
        if r.get("status") == "infra_error":
            continue  # transport failure, not a model attempt — don't show as a failure
        sd, outcome = score_prediction(r, tol)
        gt = ChartData.model_validate_json(r["gt"])
        pred = (
            ChartData.model_validate_json(r["raw"], strict=True)
            if outcome["raw_schema_valid"]
            else None
        )
    ...
    lines = [
        f"# Worst {min(n, len(scored))} predictions — {pred_path.parent.name}",
        f"\n_Sorted by cell accuracy (within {tol:.0%}). {len(scored)} scored total._\n",
    ]
    for rank, (acc, _sd, r, gt, pred) in enumerate(scored[:n], 1):
        meta = r.get("meta") or {}
...
def main():
    ...
    p.add_argument(
        "--out", default="", help="output markdown (default FAILURES.md next to predictions)"
    )
    args = p.parse_args()
