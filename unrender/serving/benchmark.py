...
def check_split(ids: list[str], final: bool) -> None:
    ...
    if final:
        raise ValueError("Common300 is historical diagnostic data, not an eligible final study")
...
async def run(args: argparse.Namespace) -> dict:
    ...
    ledger = RunLedger(args.out, metadata, schedule)
    ledger.export()  # every scheduled input exists durably before any network dispatch
    ...
    async with httpx.AsyncClient(
        ...
        async def one(index: int, scheduled: float) -> None:
            ...
            row = {
                "id": sample.id,
                "image_sha256": hashlib.sha256(images[index]).hexdigest(),
                "gt": sample.gt_json,
                "meta": sample.meta,
            ...
            if unsafe.is_set():
                ...
                try:
                    ...
                    before = time.perf_counter()
                    validity, _ = assess(result.raw, result.finish_reason, result.stream_complete)
                    row["strict_valid"] = validity["accepted"]
                    if not validity["accepted"]:
                        row.update(status="model_invalid", error="model_output_invalid")
